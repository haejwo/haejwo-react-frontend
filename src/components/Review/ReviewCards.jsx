...
export default function ReviewCards({ COdata, review }) {
    ...
    return (
        <div className='mt-4'>
            <p className='text-xl font-bold my-3 flex items-center'><BsPencilSquare className='mr-2'/> {review && review.length} 개의 리뷰</p>
            <div className='w-[364px] flex my-4 overflow-x-auto whitespace-nowrap scrollbar-none'>
                {review && COdata && review.map((list, idx) => (
                <div key={idx} className='mr-4 flex flex-col bg-zinc-100 p-4 rounded-md w-80'>
