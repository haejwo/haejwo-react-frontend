...
export default function ReviewCards({ COdata, review }) {
    ...
    return (
        <div className='mt-4 w-[364px]'>
            <p className='text-xl font-bold my-3 flex items-center text-zinc-600'><BsPencilSquare className='mr-2'/> {review ? review.length : 0} 개의 리뷰</p>
            <div className='w-full flex my-4 overflow-x-auto whitespace-nowrap scrollbar-none'>
                ...
                <div key={idx} className='mr-6 flex flex-col bg-zinc-100 p-4 rounded-md min-w-[364px]'>
                    ...
                    <div className='whitespace-pre-wrap overflow-y-auto max-h-48 scrollbar-none text-lg'>{list.comment}</div>
                </div>
                ))}
                {!review && 
                <div>리뷰가 없습니다</div>
