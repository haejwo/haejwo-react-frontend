...
export default function Less20() {
    ...
    const [moveHelp, setMoveHelp] = useState(false);
    const handleToggle = (helps) => { setMoveHelp(helps) };
    const [checked, setChecked] = useState(false);
    const handleChange = () => setChecked((prev) => !prev);
    const handleSave = () => {
        dispatch(savePacking(moveChoice));
        dispatch(saveSupport(moveHelp));
    };
        ...
        <div className='flex flex-col items-center'>
            <div className='w-full flex flex-col items-center p-4'>
                ...
                <div className='w-full flex justify-center my-4'>
                    ...
                    <button onClick={() => handleClick('SEMIPACKING')}
                        className={ moveChoice === 'SEMIPACKING' ? 
                        'w-1/3 font-semibold text-brand border py-2 border-yellow-200 bg-yellow-100' : 
                        'w-1/3 font-semibold text-zinc-500 py-2 border border-zinc-200' }>반포장</button>
                    <button onClick={() => handleClick('PACKING')}
                        className={ moveChoice === 'PACKING' ? 
                        'w-1/3 font-semibold text-brand border py-2 border-yellow-200 bg-yellow-100' : 
